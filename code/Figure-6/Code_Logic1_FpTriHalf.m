...
% This code is for Logic 1 with parameters from the 2016 paper 
% (De Mot et al.,Biophysical Journal, Volume 110, 2016, 710-722)
% The Fp value is the midpoint of the tristable region
...
for i = 1:n_cells
    ...
    initial_conditions(idx+4) = 0.0619;% Fs1=0.066; For centre of Tri Region, Fs = 0.0619
end
...
% Adjust layout
sgtitle('Logic 1, Deterministic, MT, Fp = 0.0619');
tight_layout();
...
for i = 1:n_cells
    % dG1/dt
    dydt(1,i) = ((vsg1*ERK1(i).^r/(Kag1.^r+ERK1(i).^r)) + ...
        (vsg2*G1(i).^s/(Kag2.^s+G1(i).^s)))*(Kig.^q/(Kig.^q+N1(i).^q)) - ...
        kdg*G1(i);
    
    % dN1/dt
    dydt(2,i) = ((vsn1*Kin1.^u/(Kin1.^u+ERK1(i).^u)) + ...
        (vsn2*N1(i).^v/(Kan.^v+N1(i).^v)))*(Kin2.^w/(Kin2.^w+G1(i).^w)) - ...
        kdn*N1(i);
    
    % dFR1/dt
    dydt(3,i) = vsfr1*Kifr/(Kifr+N1(i)) + (vsfr2*G1(i))/(Kafr+G1(i)) - ...
        kdfr*FR1(i);
    
    % dERK1/dt
    dydt(4,i) = va*FR1(i)*Fp(i)/(Kd+Fp(i))*((1-ERK1(i))/(Ka+1-ERK1(i))) - ...
        (vi*ERK1(i))/(Ki+ERK1(i));
