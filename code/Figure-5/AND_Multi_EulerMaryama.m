...
% stochEM_sigma0o01_V1_Multi_Unpert_AND.mat, stochEM_sigma0o01_V1_Multi_25pcInd_AND.mat 
...
%sa0 = 120-0.25*120;     % Indirect perturbation
    ...
    dB = kb0+ksb.*(s./sb0).^nsb./(1+(s./sb0).^nsb).*kab./(1+(A./ab0).^nab) ...
        .*kbb.*(B./bb0).^nbb./(1+(B./bb0).^nbb)-gb.*B;
...
function [A, B] = euler_maruyama(EMT_AND, A0, B0, s, sigma_A, sigma_B, dt, T)
    ...
    for n = 1:(N - 1)
        ...
        A(n + 1) = A(n) + dA * dt + sigma_A * dW_A * A(n) ;
        B(n + 1) = B(n) + dB * dt + sigma_B * dW_B * B(n) ;
    end
end
...
sigma_A = 0.01;        % Noise level for A
sigma_B = 0.01;        % Noise level for B
dt = 0.01;          % Time step
T = 1000.0;         % Total time


sig = [110,120,130,140,150,160,170,180];    % Signal (S) value 
...
plot(sortedBSSS(:,1),sortedBSSS(:,2),'.')
hold on
plot(sortedBUSS(:,1),sortedBUSS(:,2),'.')

plot(s_sig,B_sig,'o')

%save stochEM_sigma0o01_V1_Multi_Unpert_AND.mat A_sig B_sig s_sig
